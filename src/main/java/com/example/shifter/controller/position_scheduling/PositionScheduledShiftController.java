...
@Slf4j
@RestController
@RequestMapping("/api/scheduling/scheduled_shift")
@RequiredArgsConstructor
@Validated
public class PositionScheduledShiftController {
    ...
    public ResponseEntity<ApiResponse<List<EmployeeShiftDTO>>> getAllSchedules() {
        log.info("GET /api/scheduling/scheduled_shift/positions - Getting all scheduled shifts with positions");

        try {
            List<EmployeeShiftDTO> responses = scheduledShiftService.getAllShiftsWithPositions();
    ...
    public ResponseEntity<ApiResponse<List<EmployeeShiftDTO>>> getSchedulesByEmployee(
            @PathVariable Long employeeId) {
        log.info("GET /api/scheduling/scheduled_shift/positions/employee/{} - Getting schedules for employee", employeeId);

        try {
            List<EmployeeShiftDTO> responses = scheduledShiftService.getShiftsByEmployeeId(employeeId);
